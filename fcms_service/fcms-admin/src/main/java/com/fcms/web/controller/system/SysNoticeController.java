...
import com.fcms.system.service.ISysNoticeService;
...
public class SysNoticeController extends BaseController
{
     ...
     */
    @PreAuthorize("@ss.hasPermi('system:notice:query')")
    @GetMapping(value = "/appreciate/{noticeId}")
    public AjaxResult appreciate(@PathVariable Long noticeId)
    {
        SysNotice sysNotice = noticeService.selectNoticeById(noticeId);
        sysNotice.setPraise(sysNotice.getPraise() + 1);
        noticeService.updateNotice(sysNotice);
        return AjaxResult.success(sysNotice);
    }
